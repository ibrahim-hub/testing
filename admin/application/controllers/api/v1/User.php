    ...
    { 
        ...
        else
        {
            ...
            if(!empty($this->put('email')))
			  ...
			  if(count($user) <= 0){
				$data =	$this->db->select('phone,country_code')->from('tbl_users')->get()->result_array();
				for($i=0; $i<count($data); $i++){
					$data1	=	str_replace('+','',$data[$i]['country_code'].$data[$i]['phone']);
					$country_phone	=	str_replace('+','',$this->put('phone'));
					if($data1 == $country_phone){
						$user = $this->user_model->checkUserphonelogin($data[$i]['phone'], $this->put('password'));
						break;
					}
				}
			}
